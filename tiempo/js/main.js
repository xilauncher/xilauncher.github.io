const APP_STATE = {
  currentCity: { name: "Granada", lat: 37.1882, lng: -3.6067 }, // Localización inicial
  weatherData: null,
  aqiData: null,
  activeTab: "tab-today"
};

// Mapeo de códigos de clima (WMO) a iconos y textos
const WEATHER_CODES = {
  0: { text: "Despejado", icon: "ri-sun-fill", iconNight: "ri-moon-clear-fill" },
  1: { text: "Mayormente despejado", icon: "ri-sun-cloudy-line", iconNight: "ri-moon-cloudy-line" },
  2: { text: "Parcialmente nublado", icon: "ri-cloudy-line", iconNight: "ri-cloudy-line" },
  3: { text: "Nublado", icon: "ri-cloud-fill", iconNight: "ri-cloud-fill" },
  45: { text: "Niebla", icon: "ri-mist-line", iconNight: "ri-mist-line" },
  48: { text: "Niebla escarcha", icon: "ri-mist-line", iconNight: "ri-mist-line" },
  51: { text: "Llovizna ligera", icon: "ri-drizzle-line", iconNight: "ri-drizzle-line" },
  53: { text: "Llovizna", icon: "ri-drizzle-fill", iconNight: "ri-drizzle-fill" },
  61: { text: "Lluvia leve", icon: "ri-showers-line", iconNight: "ri-showers-line" },
  63: { text: "Lluvia moderada", icon: "ri-heavy-showers-line", iconNight: "ri-heavy-showers-line" },
  65: { text: "Lluvia fuerte", icon: "ri-heavy-showers-fill", iconNight: "ri-heavy-showers-fill" },
  71: { text: "Nieve leve", icon: "ri-snowy-line", iconNight: "ri-snowy-line" },
  73: { text: "Nieve moderada", icon: "ri-snowy-fill", iconNight: "ri-snowy-fill" },
  95: { text: "Tormenta", icon: "ri-thunderstorms-line", iconNight: "ri-thunderstorms-line" },
};
...
async function fetchWeatherData() {
  ...
  // API Principal: Clima + Pronóstico
  const weatherUrl = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,uv_index_max&timezone=auto`;
  
  // API Secundaria: Calidad del Aire (AQI Europeo)
...
async function searchGeocoding(query) {
  try {
    ...
    if (data.results && data.results.length > 0) {
      dropdown.innerHTML = data.results.map(city => `
...
function requestUserLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      // Usamos Reverse Geocoding para obtener el nombre
      try {
         const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`);
         const data = await res.json();
         const cityName = data.address.city || data.address.town || data.address.village || "Tu Ubicación";
...
function renderUI() {
  ...
  document.getElementById("tempFeels").textContent = Math.round(current.apparent_temperature);
  document.getElementById("tempMax").textContent = Math.round(daily.temperature_2m_max[0]);
  document.getElementById("tempMin").textContent = Math.round(daily.temperature_2m_min[0]);
  ...
  document.getElementById("currentIcon").className = `weather-main-icon ${isNight ? weatherCode.iconNight : weatherCode.icon}`;
  
  const card = document.querySelector(".current-weather-card");
  isNight ? card.classList.add("is-night") : card.classList.remove("is-night");

  // 2. Métricas
  document.getElementById("metricHumidity").textContent = `${current.relative_humidity_2m}%`;
  document.getElementById("metricWind").textContent = `${Math.round(current.wind_speed_10m)} km/h`;
  document.getElementById("metricUV").textContent = Math.round(daily.uv_index_max[0]);
  
  // 3. Calidad del Aire (AQI Europeo)
  ...
  if (aqiVal <= 20) {
    aqiBox.style.backgroundColor = "rgba(16,185,129,0.1)"; aqiBox.style.color = "#065f46";
    aqiDesc.textContent = "Calidad del aire Excelente. Perfecto para deporte al aire libre.";
  } else if (aqiVal <= 50) {
    aqiBox.style.backgroundColor = "rgba(245,158,11,0.1)"; aqiBox.style.color = "#92400e";
    aqiDesc.textContent = "Calidad del aire Razonable. Ligera contaminación detectada.";
  } else {
    aqiBox.style.backgroundColor = "rgba(244,63,94,0.1)"; aqiBox.style.color = "#9f1239";
...
function initTheme() {
  if (localStorage.getItem("tiempo_theme") === "dark" || (!localStorage.getItem("tiempo_theme") && window.matchMedia("(prefers-color-scheme: dark)").matches)) {
